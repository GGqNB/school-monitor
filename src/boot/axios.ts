...
export default boot(({ store, router }: { store: any; router: Router }) => {
  ...
  //       || error.response.status === 422 && error.request.responseURL == USER.REGISTRATION.url 
  //       || error.response.status === 400 && error.request.responseURL == USER.REGISTRATION.url
  //       || error.response.status === 500 && error.request.responseURL == USER.REGISTRATION.url
